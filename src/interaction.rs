...
impl Interaction {
    ...
    pub fn donate(&self) -> Result<(), IntentsError> {
        ...
        unsafe { ffi::inx_interaction_donate(self.as_ptr(), donate_callback, context) };
        receiver.recv().map_err(|error| {
            IntentsError::framework(format!(
                "interaction donation callback channel dropped: {error}"
            ))
        })?
    }
...
unsafe extern "C" fn donate_callback(context: *mut c_void, error: *const c_char) {
